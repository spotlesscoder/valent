                            ...
                            gconstpointer      user_data)
{
  ...
  dest = valent_context_get_file (dest_dir, "image.png", FALSE);
                              ...
                              gconstpointer      user_data)
{
  ...
  dest = valent_context_get_file (dest_dir, "image.png (1)", FALSE);
  g_assert_true (g_file_query_exists (dest, NULL));
  g_clear_object (&dest);

  dest = valent_context_get_file (dest_dir, "image.png (2)", FALSE);
  g_assert_true (g_file_query_exists (dest, NULL));
  g_clear_object (&dest);
}
