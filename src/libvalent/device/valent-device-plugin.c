 ...
 *     [method@Valent.Context.create_plugin_settings] for more information.
 *
 * Since: 1.0
 */
  ...
  if (priv->context == NULL)
    {
      ...
      context = valent_device_ref_context (priv->device);
...
valent_device_plugin_get_settings (ValentDevicePlugin *plugin)
{
  ...
  if (priv->settings == NULL)
    {
      priv->settings = valent_context_create_plugin_settings (priv->context,
                                                              priv->plugin_info,
                                                              PLUGIN_SETTINGS_KEY);
    }
