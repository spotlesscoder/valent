...
valent_notification_dialog_sync (ValentNotificationDialog *self)
{
  ...
  /* Check if the device action for replying is enabled */
  if (self->device != NULL && repliable)
  ...
  /* Check if a reply is ready to be sent */
  if (enabled)
    {
  ...
  gtk_widget_action_set_enabled (GTK_WIDGET (self), "dialog.reply", enabled);
}
...
static void
dialog_reply_action (GtkWidget  *widget,
                     const char *action_name,
                     GVariant   *parameter)
{
  ValentNotificationDialog *self = VALENT_NOTIFICATION_DIALOG (widget);
...
valent_notification_dialog_class_init (ValentNotificationDialogClass *klass)
{
  ...
  gtk_widget_class_install_action (widget_class, "dialog.reply", NULL, dialog_reply_action);
